...
void loop()
{
  ...
  if (counter >= aggregate) {
    ...
    // Construct the payload UR/MIN/AVG/MAX
    String payload = "UR/";
    payload += String(minVal, DEC);
    payload += "/";
    payload += String(avgVal, DEC);
    payload += "/";
    payload += String(maxVal, DEC);

    byte payloadBytes[16];
    payload.getBytes(payloadBytes, payload.length() + 1);

    switch (LoRaBee.send(1, payloadBytes, payload.length()))
    {
      case NoError:
